// ── Field Page Hamburger Navigation ──────────────────────────────
(function () {
    'use strict';

    const currentPath = location.pathname.split('/').pop() || 'field-home.html';
    ...
    const pages = [
        { href: 'field-home.html', icon: '📍', label: 'Standort erfassen' },
        { href: 'field-records.html', icon: '📝', label: 'Feld-Einträge', needsParams: true },
        { href: 'field-map.html', icon: '🗺️', label: 'Feld-Karte', needsParams: true },
    ];

    // Home link
    const homeLink = document.createElement('a');
    homeLink.href = 'index.html';
    homeLink.innerHTML = '<span class="nav-icon">🏠</span> Übersicht';
    drawer.appendChild(homeLink);

    const divTop = document.createElement('div');
    divTop.className = 'nav-divider';
    drawer.appendChild(divTop);
    ...
    pages.forEach(p => {
        ...
        if (!isActive) {
            ...
            a.addEventListener('click', e => {
                e.preventDefault();
                if (p.needsParams) {
                    let name = paramName, dog = paramDog;
                    ...
                    if (!name || !dog) {
                        ...
                        if (toast) { toast.textContent = 'Bitte zuerst Name und Hund auswählen'; toast.className = 'toast error'; setTimeout(() => toast.className = 'toast hidden', 2500); }
                        return;
                    }
    ...
    document.addEventListener('keydown', e => { if (e.key === 'Escape' && drawer.classList.contains('open')) toggle(); });
